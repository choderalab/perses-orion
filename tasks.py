...
@task
def flake8(ctx):
    run("flake8 --max-line-length 100 cubes")


@task
def update_manifest(ctx):
    """
    Updates manifest.json with the correct version of perses-orion
    """
    spec = loads(open("manifest.json", "r").read())
    sys.path.append(os.path.dirname(__file__))
    import cubes
    spec["version"] = cubes.__version__
...
def docs(ctx):
    curdir = os.getcwd()
    run("cube_doc cubes docs/source")
    run("floe_doc cubes floes docs/source")
    os.chdir("docs")
    if sys.platform == "win32":
        run("make.bat html")
    else:
...
def clean(ctx):
    ...
    egg_path = "{}.egg-info".format("perses-orion".replace("-", "_"))
